...
async function fetchShows(): Promise<void> {
  ...
  genres.value = [...new Set(tvShows.value?.map((show: TvShow) => show.genres).reduce(function (a, b) {
    return a.concat(b);
  }))].sort();
...
<template>
  <div class="content-wrapper" v-for="data in displayData" :key="data.genre">
    ...
    <div class="tile-wrapper">
      <Tile v-for="tvShow in data.shows" :key="tvShow.id" :tvShow="tvShow" :imgSrc="tvShow.image?.medium"/>
    </div>
  </div>
</template>
